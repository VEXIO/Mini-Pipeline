...
module Sim_Top(
    ...
    output wire [31:0] data_in, data_out, rega, addr_out, inst_out, pc_out, jump_addr_out
);
    ...
    Multi_CPU U1(.clk(CPU_clk), .reset(rst), .inst_out(inst), .INT(INT), .PC_out(PC), .mem_w(mem_w), .Addr_out(Addr_out), .Data_in(data_in), .Data_out(Data_out), .state(State), .CPU_MIO(), .MIO_ready(1'b1), .Reg_A_out(Reg_A_out), .jump_addr_out(jump_addr_out), .debug(debug));

    assign data_out = Data_out;
    assign rega = Reg_A_out;
