...
module vgac (
    ...
    output reg [3:0] r, // red, green, blue colors, 4-bit for each
    output reg [3:0] g,
    output reg [3:0] b,
    output reg hs, // horizontal and vertical synchronization
    output reg vs
);

    reg [9:0] h_count = 10'b0; // h_count: vga horizontal counter (0-799 pixels)
    ...
    reg [9:0] v_count = 10'b0; // v_count: vga vertical counter (0-524 lines)
    always @ (posedge vga_clk or negedge clrn) begin
        if (!clrn)
            v_count <= 10'h0;
